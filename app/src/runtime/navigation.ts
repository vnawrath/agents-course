...
import { atom, Box, type Editor } from 'tldraw'
...
export function fitSlide(editor: Editor) {
  ...
  if (bounds) editor.zoomToBounds(bounds, { inset: 32, animation: undefined })
}
...
export function goToSlide(editor: Editor, index: number, step = 0) {
  const i = Math.max(0, Math.min(slides.length - 1, index))
  const slide = slides[i]
  if (!slide) return
  const n = stepCount(i)
  const s = step < 0 ? Math.max(0, n + step) : Math.min(n - 1, step)
  if (editor.getCurrentPageId() !== slide.pageId) editor.setCurrentPage(slide.pageId)
  stepAtom.set(s)
  editor.selectNone()
  fitSlide(editor)
...
export function next(editor: Editor) {
  ...
  if (s < stepCount(i) - 1) goToSlide(editor, i, s + 1)
